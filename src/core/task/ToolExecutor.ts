...
import { ToolExecutionStrategies } from "./tools/utils/ToolExecutionStrategies"
...
export class ToolExecutor {
	...
	private async handleCompleteBlock(block: ToolUse, config: any): Promise<void> {
		...
		// Handle legacy tools that aren't fully self-managed yet
		switch (block.name) {
			...
			case "new_task":
			case "condense":
				await ToolExecutionStrategies.executeToolWithValidation(block, this.coordinator, config, this.pushToolResult)
				break
			default:
				await ToolExecutionStrategies.executeSimpleTool(block, this.coordinator, config, this.pushToolResult)
				break
		}
